...
**RELEASE**: planner go (`a_target > 0.2` with lead pull/gap growth, or no lead), a measured lead
that opens the latched gap by >0.3 m while maintaining >0.5 m/s relative recession for 0.5 s, or
state exit. Ramp to 0 at J_GO; a stale negative planner stop may not pin a clearly departing lead,
a brief radar excursion may not release the hold, and a stationary far rest may not initiate
gap-closing motion (TCS path untouched).

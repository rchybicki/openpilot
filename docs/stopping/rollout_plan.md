...
| 3.C2 — Comfort: terminal disc-grab | IMU metric (gating, 30 m/s³) + StopReq gate 0.01 (openpilot owns terminal) | IMU wired (livePose.accelerationDevice.x) | reduce IMU settle jerk; crank threshold down | **measurable + experiment deployed** (4d9cad7730): IMU shows grab ~9× worse than aEgo (20–40 m/s³). SCC handoff NOT the culprit (grab pervasive both arms). Gate→0.01 enables the next step: an openpilot anti-stiction terminal pre-release | 2026-06-13 |

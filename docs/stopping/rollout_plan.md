...
| 3.A — StopReq stage A | `STOPREQ_LATCH = True`, gate `STOP_REQ_MAX_SPEED 0.01 → 0.04`, release `STOPREQ_RELEASE_SPEED 0.10` | Stage 1 exit (corpus established → actuator-path identification ordering satisfied); independent of stage 2 outcome | Sub-stage C passed, or deliberate hold at an earlier sub-stage | **deployed + on-road validated** (390054594e; 2 routes / 13 holds: AVH_LAMP + PBRAKE_ACT False on every frame, 0 faults during holds, latch never held on a rolling car) | 2026-06-13 |
| 3.C1 — Comfort: unnecessary harsh approach | P1 kinematic approach-decel cap (≤ 0.5 m/s² while lead gap > 2 m, released when kinematically required) in longcontrol + stopping_controller | Stage 1 exit; measurable (command-based, engaged-masked, validated) | Approach-harsh rate ↓ on-road over subsequent cycles; no under-braking | **deployed** (sim: approach-harsh ~halved both plants, no under-braking; on-road confirmation next cycle) | 2026-06-13 |
| 3.C2 — Comfort: terminal disc-grab | (held) | needs a faithful metric first | — | **blocked — not measurable yet** (wheel-aEgo quantizes to ~0 at standstill; command SCC-blind under StopReq-A). Next: wire IMU long-accel into the eval, THEN crank/iterate. Command lever ~exhausted → likely an SCC-handoff fix, not deeper command caps | 2026-06-13 |

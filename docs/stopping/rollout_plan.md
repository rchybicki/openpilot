...
| 3.A — StopReq stage A (staged) | `STOPREQ_LATCH = True`, gate `STOP_REQ_MAX_SPEED 0.01 → 0.04`, release `STOPREQ_RELEASE_SPEED 0.10` | Stage 1 exit (corpus established → actuator-path identification ordering satisfied); independent of stage 2 outcome | Sub-stage C passed, or deliberate hold at an earlier sub-stage | **staged** (constants flipped + tests green; awaiting human commit/deploy + parking-lot session) | 2026-06-13 |

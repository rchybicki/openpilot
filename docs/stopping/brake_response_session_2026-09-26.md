...
not disable it. The hook never commands positive acceleration, but after it releases, normal cruise CAN accelerate.
The only cue is the on-screen banner; there is no tone.
...
- Choose the LAST-PRESS point from the measured geometry, so that everything after the press (B2 worksheet: hold,
...
- FrogPilot -> WHEEL CONTROLS: set "Distance Button", "Distance Button (Long Press)" and "Distance Button (Very Long Press)" to
  "No Action". The hook needs both long mappings at "No Action"; the short mapping is set too so that a short press
  does nothing.
...
- Start from the start of the usable straight after A2 and B4 checks. Engage at or below 30 km/h; do not resume an old higher target.
  Check that the displayed set speed IS 30 km/h before any hold. If it differs, cancel and park. No car ahead, wheel
...
- Wait for a steady speed. The screen shows `STEP TEST READY` when every condition held for 2 s (measured speed, not
  ...
  on-screen distance button feeds the same signal). No phone or chat use; nobody sends the driver messages.
- Only if READY shows BEFORE the LAST-PRESS point: hold the wheel distance button for about 1.5 s (less than 2.5 s),
  then release. Do not press or hold while you wait for READY. If READY is late or does not come, do not press;
  continue manually.
- If a condition fails while you hold, the hold is cancelled. Release. Do not try again this run.
- The screen shows `STEP TEST ACTIVE`: openpilot requests -0.5 m/s^2 for up to 3 s. How the car actually responds is
  what this trial measures; do not expect exact gentle braking.
- When the screen shows `STEP COMPLETE` or `STEP ABORTED`, brake normally to a stop. Do not wait for cruise to
  pick up speed again.
- The brake pedal, gas pedal or cruise cancel ends the hook's authority at once. An input fault can instead hold the
  previous braking command for a moment; brake if needed.

A4. After the attempt
- Park. Do not press the distance button again. Do not drive the straight again. The screen says
  `park and review; do not repeat`. Then follow B5 (teardown) before driving anywhere.
...
or vehicle; any change in grip or visibility; an unexpected restart; `STEP TEST ARMED` or `READY` shown again after
the attempt or after `STEP TEST DONE` (a restart re-armed the hook); a waiting reason that does not clear.
...
- `selfdrive/controls/lib/longcontrol.py:766` arms the hook when `/data/identification_hook.arm` exists at LongControl
  construction (controlsd start, i.e. each ignition-on). The arm file persists. Removing it does not disarm a running
  controlsd. A restart with the file present resets the trial count.
- `selfdrive/controls/lib/identification_hook.py:32` `MAX_TRIALS = 1` per hook instance; `:37` first profile
  `step -0.5` for 3.0 s. Later profiles stay defined but unreachable. `:25` start band 7-11 m/s (first smoke targets
  8.33 m/s); `:26` the command ends early at vEgo <= 4.5 m/s; `:31` release bound 0.8 m/s^3 after the trial; `:205` a
  pedal or disengagement removes the hook bound on the same frame; `:243` a failed precondition during a hold needs
  a new press.
- `selfdrive/controls/controlsd.py:55,211` the banner uses `alertDebug`; `fullupdate.sh:63,176` (staging banner and
  ...
  can end controlsd (a restart, an SCC12 gap, and a re-arm while the file exists); this was not reproduced on the car.
...
- `frogpilot/controls/frogpilot_card.py:84,95`: the long action fires at 0.5 s of hold; the on-screen button is merged
  into `distancePressed`.
- Mappings: keys `DistanceButtonControl`, `LongDistanceButtonControl`, `VeryLongDistanceButtonControl`
...
Assumptions: start from rest inside the usable straight; constant 8.33 m/s through 2 s qualification, 1.5 s hold, the
3 s profile (no credit for its braking) and the 0.625 s nominal release; then 2 s to manual braking with the listed
...
EXPECT_MAP="${EXPECT_MAP:-}"          # e.g. 2/1/6 at B5.4 (the values of the first site snapshot); empty = record only
...
for k in DistanceButtonControl LongDistanceButtonControl VeryLongDistanceButtonControl InitialSetSpeed LongitudinalManeuverMode IsOnroad IsOffroad; do
...
1. Enable patch (not applied): `~/.route_sync/corpus/calibration_session_prep_20260925/build/enable_identification_hook.patch`
   (one line, `False` -> `True`); reverse: `disable_identification_hook.patch`. `fullupdate.sh` resets to the pushed
   branch, so the host commits and pushes the enable change, and pushes the reverse as soon as the session ends.
2. Ignition OFF, `EXPECT_MAP=0/0/0 EXPECT_INITIAL=30 bash b3.sh` PASS. Deploy: `ssh -tt comma 'cd /data/openpilot && ./fullupdate.sh'` (on timeout at once
...
3. After the reboot: `EXPECT_FLAG=True EXPECT_MAP=0/0/0 EXPECT_INITIAL=30 bash b3.sh` PASS, HEAD = enable commit.
4. Ignition still OFF: `ssh comma 'touch /data/identification_hook.arm'`. Then ignition ON, parked.
5. Confirm from the file, not a subscriber:
   `ssh comma 'grep -h "identification hook constructed" /data/log/swaglog.* | tail -n 2'`: the newest line (check its
   `created` time) must be from this start and show `armed=True`. Then the driver follows A3.
...
Otherwise:
1. Park and switch cruise off. Restore the three mappings and Initial Set Speed in the UI to the restore record;
   do not cycle the ignition before step 2.
   Nonzero long mappings block a new start (an extra interlock only; this alone is never a teardown).
2. `ssh comma 'rm -f /data/identification_hook.arm && test ! -e /data/identification_hook.arm'` must succeed (same fallback).
...
Nobody drives home with the flag True or the arm file present.

...
# -- cycle 50 (2026-09-05): the service ENTERS on the model's trajectory stop intent with no lead of any kind ----------
def _intent_frames(monkeypatch, n, *, model_stop_d=3.0, lead_status=False, lead2_status=False, a_target=-0.9, v_ego=1.8,
                   flag=True, stop_d_after=None, experimental_mode=True):
  monkeypatch.setattr(stopping_flags, "SERVICE_MODE", "LIVE")
  monkeypatch.setattr(stopping_flags, "NOLEAD_ATTRIBUTED_SAFETY", "live" if flag else "off")
  lc = LongControl(DummyCarParams())
  owning = []
  for i in range(n):
    msd = model_stop_d if (stop_d_after is None or i < stop_d_after) else -1.0
    lc.update(active=True, CS=DummyCarState(v_ego=v_ego, a_ego=-0.5, standstill=False),
              a_target=a_target, should_stop=False, model_should_stop=False,
              distance_to_stop_target_m=-1.0, accel_limits=LIMITS,
              frogpilot_toggles=DummyFrogPilotToggles(),
              lead_status=lead_status, lead_v=0.0, lead_d_rel=8.0 if lead_status else 0.0,
              lead_track_id=-1, lead_model_prob=0.5 if lead_status else 0.0, a_target_trajectory=a_target,
              lead2_status=lead2_status, lead2_v=0.0, lead2_d_rel=20.0 if lead2_status else 0.0,
              model_stop_d=msd, experimental_mode=experimental_mode)
    owning.append(bool(lc._service_live_owning))
  return lc, owning


def test_trajectory_intent_enters_the_service_after_the_persistence(monkeypatch) -> None:
  lc, owning = _intent_frames(monkeypatch, 60)
  first = owning.index(True)
  assert 18 <= first <= 24            # NOLEAD_INTENT_PERSIST_S = 0.20 s of continuous intent, no earlier
  assert lc._service_shadow_svc.phase != Phase.INACTIVE


def test_trajectory_intent_is_refused_with_any_reported_lead_or_without_planner_braking(monkeypatch) -> None:
  _, owning = _intent_frames(monkeypatch, 60, lead_status=True)          # vision-only lead (radarTrackId -1)
  assert not any(owning)
  _, owning = _intent_frames(monkeypatch, 60, lead2_status=True)
  assert not any(owning)
  _, owning = _intent_frames(monkeypatch, 60, a_target=-0.1)             # the planner is not braking
  assert not any(owning)
  _, owning = _intent_frames(monkeypatch, 60, model_stop_d=-1.0)         # no model stop in the plan
  assert not any(owning)
  _, owning = _intent_frames(monkeypatch, 60, v_ego=2.6)                 # above the governor band
  assert not any(owning)
  _, owning = _intent_frames(monkeypatch, 60, flag=False)                # flag off = today's behaviour
  assert not any(owning)
  _, owning = _intent_frames(monkeypatch, 60, experimental_mode=False)   # ACC mode: the e2e trajectory is not the plan
  assert not any(owning)


def test_trajectory_intent_falling_edge_is_held_then_released(monkeypatch) -> None:
  lc, owning = _intent_frames(monkeypatch, 140, stop_d_after=60)
  assert owning[59]
  assert all(owning[60:60 + 35])      # NOLEAD_INTENT_HOLD_S = 0.40 s: no chatter exit on a one-frame intent drop
  assert not lc._nolead_intent_hold_s > 0.0 or not owning[-1]


def test_trajectory_intent_survives_a_two_frame_flicker_at_its_onset(monkeypatch) -> None:
  # the plan flickers when the intent first appears (census 2026-09-05): a 2-frame drop decays the persistence
  # instead of restarting it, so the service still enters ~0.3 s after the intent began
  monkeypatch.setattr(stopping_flags, "SERVICE_MODE", "LIVE")
  monkeypatch.setattr(stopping_flags, "NOLEAD_ATTRIBUTED_SAFETY", "live")
  lc = LongControl(DummyCarParams())
  owning = []
  for i in range(60):
    msd = -1.0 if i in (10, 11) else 3.0
    lc.update(active=True, CS=DummyCarState(v_ego=1.8, a_ego=-0.5, standstill=False),
              a_target=-0.9, should_stop=False, model_should_stop=False, distance_to_stop_target_m=-1.0,
              accel_limits=LIMITS, frogpilot_toggles=DummyFrogPilotToggles(), lead_status=False, lead_v=0.0,
              lead_d_rel=0.0, lead_track_id=-1, lead_model_prob=0.0, a_target_trajectory=-0.9, model_stop_d=msd,
              experimental_mode=True)
    owning.append(bool(lc._service_live_owning))
  assert 22 <= owning.index(True) <= 30

...
# -- cycle 50 (2026-09-05): the NO-LEAD branch ------------------------------------------------------------------------
def _frames_nolead(monkeypatch, flag, n, *, model_stop_d=3.0, any_lead=False, fcw=False, a_target=-1.3, a_traj=-0.6, v=1.8,
                   ghost_gap=None, nolead_flag=True, should_stop=True, wire=None, force_coast_accel=None, nolead_intent=True,
                   lead_after=None, wire_after_lead=-0.06):
  """No radar lead at all: the planner braking a_target (its final demand) for a model stop model_stop_d ahead while its
  own trajectory lane asks a_traj. wire: the legacy chain's value this frame (the takeover seed and the baseline; None =
  it follows the planner's demand, as the recorded no-lead stops show); wire_after_lead: the legacy value once the
  fresh radar object is present (2041 s3: the MPC coasted at -0.06 while the service's a_kin would brake -0.9).
  ghost_gap: a lead accepted at that gap that vanished right before the window (the context's 2 s dropout hold).
  lead_after: from this frame a fresh radar object (id 44) appears 30 m ahead closing fast (the 2041 s3 crossing car)."""
  from openpilot.selfdrive.controls.lib.stop_context import StopContext
  from openpilot.selfdrive.controls.lib.stopping_service import StoppingService, ATTR_LIVE_DWELL_S
  _law_flag(monkeypatch, flag)
  monkeypatch.setattr(stopping_flags, "NOLEAD_ATTRIBUTED_SAFETY", "live" if nolead_flag else "off")
  ctx, s = StopContext(), StoppingService()
  if ghost_gap is not None:
    for _ in range(50):
      ctx.update(v_ego=v, a_ego=-0.5, a_cmd=-0.5, lead_status=True, lead_v=0.0, lead_d_rel=ghost_gap, lead_track_id=9, standstill=False, dt=0.01)
  wires, dbg = [], []
  for i in range(n):
    phantom = lead_after is not None and i >= lead_after
    wire_i = (a_target if wire is None else wire) if not phantom else wire_after_lead
    sig = ctx.update(v_ego=v, a_ego=-0.5, a_cmd=-0.5, lead_status=phantom, lead_v=-5.5 if phantom else 0.0,
                     lead_d_rel=30.0 if phantom else None, lead_track_id=44 if phantom else None, standstill=False, dt=0.01)
    r = s.update(engaged=True, v_ego=v, a_ego=-0.5, a_target=a_target, should_stop=should_stop and not phantom, dts_planner=None,
                 planner_min_limit=-3.5, signals=sig, lead_status=phantom, lead_v=-5.5 if phantom else 0.0, increased_stopped_distance=0.3,
                 dt=0.01, wire_accel=wire_i, a_target_trajectory=a_traj, lead_a=0.0, lead2=None, fcw=fcw, model_stop_d=model_stop_d,
                 any_lead=any_lead or phantom, nolead_intent=nolead_intent and not phantom, force_coast_accel=force_coast_accel)
    wires.append(r.accel)
    dbg.append(r.debug)
  return wires, dbg, ATTR_LIVE_DWELL_S


def test_nolead_live_releases_the_planner_excess_down_to_the_trajectory_lane(monkeypatch):
  # planner -1.3, its own trajectory lane -0.6, the glide ~-0.4: the release stops AT the trajectory lane (red-team CRITICAL:
  # with no radar lead that lane may be the only brake for a pedestrian / an untracked car)
  shadow, sdbg, dwell = _frames_nolead(monkeypatch, "shadow", 200)
  live, dbg, _ = _frames_nolead(monkeypatch, "live", 200)
  assert dbg[5]["attr_nolead"] and dbg[5]["attr_eligible"] and dbg[5]["attr_reason"] is None
  from openpilot.selfdrive.controls.lib.stopping_service import NOLEAD_LIVE_DWELL_S
  first_live = next(i for i, d in enumerate(dbg) if d.get("attr_live"))
  assert first_live <= int(NOLEAD_LIVE_DWELL_S * 100) + 1 and NOLEAD_LIVE_DWELL_S < dwell
  assert live[:first_live] == shadow[:first_live]
  assert all(live[k] >= shadow[k] - 1e-9 for k in range(200))     # release-only
  assert live[-1] == pytest.approx(-0.6, abs=0.02)                 # bounded by the trajectory lane, not the -0.4 glide
  assert dbg[-1]["attr_candidate"] == pytest.approx(-0.6, abs=1e-6) and dbg[-1]["a_phase"] < -0.3
  assert max(live[k + 1] - live[k] for k in range(199)) <= 0.8 * 0.01 + 1e-9
  assert sdbg[-1]["attr_plan_bound"] and sdbg[-1]["attr_unexplained"]


def test_nolead_release_is_bounded_by_the_force_coast_level_and_needs_a_valid_trajectory_lane(monkeypatch):
  live, dbg, _ = _frames_nolead(monkeypatch, "live", 150, a_traj=-0.3, force_coast_accel=-0.7)
  assert dbg[-1]["attr_candidate"] == pytest.approx(-0.7, abs=1e-6) and live[-1] == pytest.approx(-0.7, abs=0.02)
  shadow, _, _ = _frames_nolead(monkeypatch, "shadow", 100, a_traj=None)
  live, dbg, _ = _frames_nolead(monkeypatch, "live", 100, a_traj=None)
  assert live == shadow and dbg[-1]["attr_reason"] == "unusable"


def test_nolead_branch_is_fail_closed_on_a_vision_lead_fcw_or_missing_model_stop(monkeypatch):
  for kwargs, reason in (({"any_lead": True}, "vision_lead"), ({"fcw": True}, "fcw"), ({"model_stop_d": -1.0}, "no_model_stop"),
                         ({"model_stop_d": float("nan")}, "unusable")):
    shadow, _, _ = _frames_nolead(monkeypatch, "shadow", 120, **kwargs)
    live, dbg, _ = _frames_nolead(monkeypatch, "live", 120, **kwargs)
    assert live == shadow
    assert dbg[-1]["attr_nolead"] and dbg[-1]["attr_eligible"] is False and dbg[-1]["attr_reason"] == reason


def test_nolead_branch_during_a_dropout_hold_needs_the_ghost_far_beyond_the_model_stop(monkeypatch):
  live_close, dbg_close, _ = _frames_nolead(monkeypatch, "live", 120, ghost_gap=6.0)
  shadow_close, _, _ = _frames_nolead(monkeypatch, "shadow", 120, ghost_gap=6.0)
  assert dbg_close[-1]["attr_reason"] == "dropout" and live_close == shadow_close
  live_far, dbg_far, _ = _frames_nolead(monkeypatch, "live", 120, ghost_gap=33.0)
  assert dbg_far[-1]["attr_eligible"] and dbg_far[-1]["attr_live"]


def test_nolead_model_stop_is_a_remaining_distance_candidate_so_a_ghost_gap_cannot_shallow_the_glide(monkeypatch):
  _, dbg_off, _ = _frames_nolead(monkeypatch, "shadow", 60, ghost_gap=33.0, nolead_flag=False)
  _, dbg_on, _ = _frames_nolead(monkeypatch, "shadow", 60, ghost_gap=33.0, nolead_flag=True)
  assert dbg_off[-1]["a_phase"] > -0.12 and dbg_on[-1]["a_phase"] < -0.30
  assert dbg_on[-1]["d_rem"] < 6.0 < dbg_off[-1]["d_rem"]


def test_nolead_branch_absent_with_the_flag_off_or_without_intent_ownership(monkeypatch):
  on, dbg_on, _ = _frames_nolead(monkeypatch, "live", 120, nolead_flag=True, model_stop_d=-1.0)
  off, dbg_off, _ = _frames_nolead(monkeypatch, "live", 120, nolead_flag=False, model_stop_d=-1.0)
  assert "attr_nolead" not in dbg_off[-1] and dbg_on[-1]["attr_reason"] == "no_model_stop"
  assert all(on[k] >= off[k] - 1e-9 for k in range(120))            # the baseline invariant is release-only too
  # the service entered on the planner's own shouldStop (not the intent): no no-lead branch, no invariant, no release
  own, dbg_own, _ = _frames_nolead(monkeypatch, "live", 120, nolead_intent=False)
  assert "attr_nolead" not in dbg_own[-1] and own == off


def test_nolead_ownership_never_brakes_more_than_the_legacy_chain_even_on_a_fresh_radar_object(monkeypatch):
  # 2041 s3: a crossing car (30 m, closing 7 m/s) became leadOne during a no-lead approach; the service's a_kin lane
  # would brake -0.9 while the legacy chain coasted at -0.06. The baseline invariant caps every frame of the episode
  # at the legacy wire, and the episode latch keeps it in force after the object appears.
  live, dbg, _ = _frames_nolead(monkeypatch, "live", 140, a_target=-1.4, lead_after=60)
  assert dbg[59].get("attr_live") and live[59] > -1.4 + 0.3          # releasing before the object
  assert min(d["a_kin"] for d in dbg[61:70]) < -0.8                  # the phantom lane really binds in the min
  # ... but the wire never DEEPENS below the legacy chain: it only hands back toward it at the jerk limit
  assert all(live[k] >= min(live[k - 1], -0.06) - 1e-6 for k in range(61, 140))
  assert live[-1] > -0.06 - 0.05
  assert "attr_nolead" not in dbg[70]                                 # a lead present: no release branch either


def test_nolead_drift_anchor_is_frozen_at_the_first_eligible_frame(monkeypatch):
  live, dbg, _ = _frames_nolead(monkeypatch, "live", 100)
  assert dbg[0]["attr_nolead_anchor_m"] == pytest.approx(3.0) and dbg[-1]["attr_nolead_anchor_m"] == pytest.approx(3.0)
  assert dbg[-1]["attr_nolead_travel_m"] == pytest.approx(1.8 * 0.99, abs=0.05)    # v * t since the anchor
  assert dbg[-1]["attr_nolead_drift_m"] is None                                        # no wheel stop yet

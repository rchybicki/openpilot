...
DIST_V_GAP2 = [ 1.4, 1.2,  1.3,  1.2,  0.9,  0.9  ]
...
def get_T_FOLLOW(personality=log.LongitudinalPersonality.standard, v_ego = 0., exp_mode = False):
  ...
  standard_t_follow = np.interp(v_ego_kph, DIST_V_BP, DIST_V_GAP2) * 0.6
  after_dark = 1.2 if is_it_after_dark_in_poland() else 1.
  if personality==log.LongitudinalPersonality.relaxed:
    return 1.1 if exp_mode else (standard_t_follow * after_dark * 1.2)
  elif personality==log.LongitudinalPersonality.standard:
    return 1.0 if exp_mode else (standard_t_follow * after_dark)
  elif personality==log.LongitudinalPersonality.aggressive:
    return 0.9 if exp_mode else (standard_t_follow * after_dark * 0.5)
  else: #snow
    return 1.2 if exp_mode else (standard_t_follow * after_dark * 1.5)

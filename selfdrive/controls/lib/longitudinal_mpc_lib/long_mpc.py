...
def get_stopped_equivalence_factor(v_lead_raw, v_ego_raw, v_lead_distance_raw, t_follow):
  ...
  if speed_difference > 0:
    ...
    v_lead_mult = np.interp(v_lead_kph, [0.0, 40.0], [0.05, 1.0])
    distance_offset = v_lead_distance * dist_mult * min(speed_difference, 14) * v_lead_mult
    distance_offset = np.clip(distance_offset, 0, v_lead_distance)

  additional_time_offset = np.interp(lead_time, [t_follow-0.3, variable_switch_point], [-0.8, 0.4])

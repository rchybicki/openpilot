...
class LateralPlanner:
  ...
  def publish(self, sm, pm):
    ...
    # PFEIFER - DLP {{
    if self.LP.using_lane_planner:
      lateralPlan.dPathPoints = self.LP.y_pts.tolist()
      lateralPlan.psis = self.LP.lat_mpc.x_sol[0:CONTROL_N, 2].tolist()

      lateralPlan.curvatures =  (self.LP.lat_mpc.x_sol[0:CONTROL_N, 3]/self.v_ego).tolist()
      lateralPlan.curvatureRates = [float(x.item() / self.v_ego) for x in self.LP.lat_mpc.u_sol[0:CONTROL_N - 1]] + [0.0]
      lateralPlan.mpcSolutionValid = bool(self.LP.valid())
    # }} PFEIFER - DLP

...
def create_lfahda_mfc(packer, lat_active, hda_set_speed=0):
  values = {
...
def create_acc_commands(stopping_cnt, vEgo, aEgo, packer, enabled, accel, upper_jerk, lower_jerk, idx, lead_visible, set_speed, stopping, long_override, use_fca):
  ...
  scc12_values = {
    ...
    "StopReq": 1 if stopping and vEgo < 0.005 else 0,  # vEgo < 0.1 or vEgo < 0.03 and aEgo > -0.04) else 0,
    "aReqRaw": accel, #max(accel + accel * (min(stopping_cnt, 1000) / 1000), -3.5) if stopping else accel,

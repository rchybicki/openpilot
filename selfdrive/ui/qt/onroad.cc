...
void AnnotatedCameraWidget::updateState(const UIState &s) {
  ...
  bool brake_lights = sm["carState"].getCarState().getBrakeLightsDEPRECATED();
  ...
  setProperty("brakeLights", brake_lights);
...
void AnnotatedCameraWidget::drawRedText(QPainter &p, int x, int y, const QString &text, int alpha) {
  ...
  p.setPen(QColor(0x9c, 0x00, 0x00, alpha)); // set the pen to red with the provided alpha
  p.drawText(real_rect.x(), real_rect.bottom(), text);
}

  ...
  if (s->scene.world_objects_visible) {
    ...
    if (s->scene.longitudinal_control && sm.rcv_frame("radarState") > s->scene.started_frame && !frogpilot_toggles.value("hide_lead_marker").toBool()) {
      ...
      if (lead_left.getStatus()) {
        drawLead(painter, reinterpret_cast<const cereal::RadarState::LeadData::Reader &>(lead_left), frogpilotPlan, s->scene.lead_vertices[2], frogpilot_nvg->blueColor(), fs, true, speedAdjustmentFactor);
      }
      if (lead_right.getStatus()) {
        drawLead(painter, reinterpret_cast<const cereal::RadarState::LeadData::Reader &>(lead_right), frogpilotPlan, s->scene.lead_vertices[3], frogpilot_nvg->purpleColor(), fs, true, speedAdjustmentFactor);
      }
      if (lead_one_frogpilot.getStatus() && frogpilotPlan.getTrackingLead()) {
        drawLead(painter, reinterpret_cast<const cereal::RadarState::LeadData::Reader &>(lead_one_frogpilot), frogpilotPlan, s->scene.lead_vertices[0], lead_one.getStatus() ? fs->frogpilot_scene.lead_marker_color : whiteColor(), fs, false, speedAdjustmentFactor);
      } else {

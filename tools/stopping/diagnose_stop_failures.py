...
def load_events(summary: dict[str, Any], args: argparse.Namespace) -> list[EventRow]:
  ...
  for route_summary in summary["routes"]:
    ...
    for event in route_summary["events"]:
      ...
      lead_distance_stop_entry_raw = event.get("lead_distance_stop_entry_m")
      lead_distance_stop_entry = float(lead_distance_stop_entry_raw) if lead_distance_stop_entry_raw is not None else None
      lead_distance_hold_raw = event.get("lead_distance_hold_m")
      lead_distance_hold = float(lead_distance_hold_raw) if lead_distance_hold_raw is not None else None
      ...
      entry_stop_jerk_raw = event.get("entry_stop_jerk_mps3")
      entry_stop_jerk = float(entry_stop_jerk_raw) if entry_stop_jerk_raw is not None else None
      entry_stop_accel_step_raw = event.get("entry_stop_accel_step_mps2")
      entry_stop_accel_step = float(entry_stop_accel_step_raw) if entry_stop_accel_step_raw is not None else None
      entry_stop_cmd_jerk_raw = event.get("entry_stop_cmd_jerk_mps3")
      entry_stop_cmd_jerk = float(entry_stop_cmd_jerk_raw) if entry_stop_cmd_jerk_raw is not None else None
      entry_stop_cmd_step_raw = event.get("entry_stop_cmd_step_mps2")
      entry_stop_cmd_step = float(entry_stop_cmd_step_raw) if entry_stop_cmd_step_raw is not None else None
      ...
      using_lead_hold_gap = lead_distance_hold is not None
      rollout_excess = (not using_lead_hold_gap) and rollout_from_2m > args.rollout_threshold_m
      lead_gap_out_of_range = (
        using_lead_hold_gap
        and (lead_distance_hold < args.min_lead_hold_distance_m or lead_distance_hold > args.max_lead_hold_distance_m)
      )
      ...
      if lead_gap_out_of_range:
        flags.append("lead_stop_gap_out_of_range")
      if jerk_high:
...
def build_markdown(summary: dict[str, Any], rows: list[EventRow], args: argparse.Namespace) -> str:
  ...
  def stop_distance_ok(row: EventRow) -> bool:
    if row.lead_distance_hold_m is not None:
      return args.min_lead_hold_distance_m <= row.lead_distance_hold_m <= args.max_lead_hold_distance_m
    return row.rollout_from_2m_m <= args.rollout_threshold_m
  ...
  no_lead_rows = [row for row in rows if row.lead_distance_hold_m is None]
  lead_hold_rows = [row for row in rows if row.lead_distance_hold_m is not None]
  rollout_ok = sum(1 for row in no_lead_rows if row.rollout_from_2m_m <= args.rollout_threshold_m)
  lead_gap_ok = sum(
    1
    for row in lead_hold_rows
    if row.lead_distance_hold_m is not None and args.min_lead_hold_distance_m <= row.lead_distance_hold_m <= args.max_lead_hold_distance_m
  )
  stop_distance_good = sum(1 for row in rows if stop_distance_ok(row))
  ...
  if lead_hold_rows:
    lines.append(
      f"- Lead-follow final gap within {args.min_lead_hold_distance_m:.2f}-{args.max_lead_hold_distance_m:.2f}m: "
      f"{lead_gap_ok}/{len(lead_hold_rows)}"
    )
  if no_lead_rows:
    lines.append(f"- No-lead rollout <= {args.rollout_threshold_m:.2f}m: {rollout_ok}/{len(no_lead_rows)}")
  lines.append(f"- Stop-distance criterion met: {stop_distance_good}/{len(rows)}")
  ...
  lines.append("")
  if flag_counts.get("lead_stop_gap_out_of_range", 0) > 0:
    lines.append("- Some lead-follow stops finish outside the target final lead-gap band; use this separately from no-lead rollout so distance tuning does not hide jerk regressions.")
  ...
  lines.append("")
  lines.append(
    "|Rank|Score|Route|Seg|Event|Source|Duration|Distance|LeadHold|Rollout2m|EntryJerk|EntryStep|EntryCmdJerk|EntryCmdStep|EndJerk|EndStep|CmdJerk|CmdStep|"
  ...
  for rank, row in enumerate(ranked[:args.top_n], start=1):
    lines.append(
      ...
      f"{format_float(row.lead_distance_hold_m, 2)}|"
      f"{format_float(row.rollout_from_2m_m, 2)}|"
      f"{format_float(row.entry_stop_jerk_mps3, 2)}|"
      f"{format_float(row.entry_stop_accel_step_mps2, 2)}|"
      f"{format_float(row.entry_stop_cmd_jerk_mps3, 2)}|"
      f"{format_float(row.entry_stop_cmd_step_mps2, 2)}|"
  ...
  lines.append(f"- min_lead_hold_distance_m: {args.min_lead_hold_distance_m}")
  lines.append(f"- max_lead_hold_distance_m: {args.max_lead_hold_distance_m}")

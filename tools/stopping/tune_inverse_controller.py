...
def summarize_variant(
  ...
  for item in rows:
    ...
    pred_lead_hold_raw = item.get("pred_lead_distance_hold_m")
    ...
    distance_flags, _distance_gate_source, _distance_metric_value = classify_stop_distance(
      pred_rollout,
      pred_lead_hold,
      max_rollout_m=max_rollout_m,
      min_lead_hold_m=min_lead_hold_m,
      max_lead_hold_m=max_lead_hold_m,
    )
...
def main() -> int:
  ...
  current_summary = summarize_variant(
    ...
    args.min_pred_lead_hold_distance_m,
    args.max_pred_lead_hold_distance_m,

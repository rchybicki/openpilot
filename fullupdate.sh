...
if params.get_bool("IsOnroad"):
  ...
  if current_state in ("", "aborted", "unsupported"):
    ...
    print("Live restart armed. Fully turn cruise control off; no screen tap is required.")
...
finish_update() {
  ...
  echo "Update staged. Fully turn cruise control off; reboot is automatic after stock SCC takeover is verified."
  print_reboot_controls
  exit 0
}
...
wait_until_safe_to_update() {
  ...
  if [ -n "$unsafe_reasons" ]; then
    if [ "$phase" != "reboot" ]; then
      echo "Staging update while on-road; fully turn cruise control off after staging to finish it."
    fi
    return
  fi
}

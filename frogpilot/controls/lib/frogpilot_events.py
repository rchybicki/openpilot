...
from openpilot.frogpilot.common.frogpilot_variables import CRUISING_SPEED, NON_DRIVING_GEARS, params_memory
...
class FrogPilotEvents:
  ...
  def update(self, v_cruise, sm, frogpilot_toggles):
    event_names = {event.name for event in sm["onroadEvents"]}
    frogpilot_event_names = {event.name for event in sm["frogpilotOnroadEvents"]}

    alerts_empty = len(event_names) == 0 and len(frogpilot_event_names) == 0
    ...
    if not self.random_event_playing and frogpilot_toggles.random_events:
      ...
      if (EventName.steerSaturated in event_names or FrogPilotEventName.goatSteerSaturated in frogpilot_event_names):
        event_choices = []
      ...
      if (EventName.fcw in event_names or EventName.stockAeb in event_names):
        event_choices = []
    ...
    self.startup_seen |= FrogPilotEventName.customStartupAlert in frogpilot_event_names

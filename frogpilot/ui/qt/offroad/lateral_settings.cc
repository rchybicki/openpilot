...
FrogPilotLateralPanel::FrogPilotLateralPanel(FrogPilotSettingsWindow *parent) : FrogPilotListWidget(parent), parent(parent) {
  ...
  std::set<QString> forceUpdateKeys = {"ForceAutoTune", "ForceAutoTuneOff", "LateralTune", "NNFF", "NNFFLite", "NudgelessLaneChange"};
  ...
  std::set<QString> rebootKeys = {"AlwaysOnLateral", "ForceTorqueController", "NNFF", "NNFFLite"};
...
void FrogPilotLateralPanel::updateToggles() {
  ...
  for (auto &[key, toggle] : toggles) {
    ...
    bool usingNNFFLite = params.getBool("LateralTune") && params.getBool("NNFFLite");

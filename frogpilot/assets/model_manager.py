...
from openpilot.frogpilot.common.frogpilot_utilities import delete_file, extract_tar, get_folder_size, load_json_file, update_json_file
...
class ModelManager:
  ...
  def download_model(self, model_to_download):
    ...
    if self.model_versions[self.available_models.index(model_to_download)] in {"v1", "v2", "v3", "v4", "v5", "v6"}:
      ...
      for index, (file_name, description) in enumerate(TINYGRAD_FILES, start=1):
        filename = f"{model_to_download}_{file_name}"
        model_path = MODELS_PATH / filename
        model_url = f"{repo_url}/Models/compiled/{filename}"

        print(f"Downloading {description} for model: {model_to_download}")
        download_file(CANCEL_DOWNLOAD_PARAM, model_path, DOWNLOAD_PROGRESS_PARAM, model_url, MODEL_DOWNLOAD_PARAM, self.session, files=len(TINYGRAD_FILES), file_number=index)
  ...
  def fetch_all_model_sizes(self, repo_url):
    ...
    model_sizes = {}
    try:
      def fetch_dir_sizes(api_url, versioned=False):
        sizes = {}
      ...
      if is_github:
        ...
        version_api_url = f"https://api.github.com/repos/{RESOURCES_REPO}/contents/Models/compiled?ref=Models"
      elif is_gitlab:
        top_api_url = f"https://gitlab.com/api/v4/projects/{repo_encoded}/repository/tree?ref=Models"
        version_api_url = f"https://gitlab.com/api/v4/projects/{repo_encoded}/repository/tree?path=Models%2Fcompiled&ref=Models"
      else:
        print(f"Unsupported repository URL: {repo_url}")
        return model_sizes

      model_sizes.update(fetch_dir_sizes(top_api_url))
      model_sizes.update(fetch_dir_sizes(version_api_url, versioned=True))

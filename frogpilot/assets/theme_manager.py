...
class ThemeManager:
  ...
  def delete_theme(theme_name):
    theme_pack_path = THEME_SAVE_PATH / "theme_packs" / theme_name
    if theme_pack_path.exists():
      delete_file(theme_pack_path)
      print(f"Deleted theme pack: {theme_name}")
      return

    wheel_path = THEME_SAVE_PATH / "steering_wheels" / theme_name
    for extension in [".gif", ".png"]:
      potential_file = wheel_path.with_suffix(extension)
      if potential_file.exists():
        delete_file(potential_file)
        print(f"Deleted steering wheel image: {potential_file.name}")

  def download_theme(self, theme_component, theme_name, asset_param):
    ...
    for extension in extensions:
      ...
      if verify_download(theme_path, theme_url, self.session):
        ...
        self.downloading_theme = False
        return
      elif self.handle_verification_failure(extension, theme_component, theme_name, asset_param, theme_path, download_path):
        self.downloading_theme = False
        return
    ...
    try:
      for branch in ["Distance-Icons", "Steering-Wheels"]:
        if is_github:
          ...
          api_url = f"https://gitlab.com/api/v4/projects/{repo_encoded}/repository/tree?ref={branch}&recursive=true"
        else:
          ...
          if branch == "Steering-Wheels":
            ...
            if local_files:
              ...
              if expected_size > 0 and local_size != expected_size:
                print(f"Wheel {theme_name} is outdated, redownloading...")
                self.download_theme("steering_wheels", theme_name, THEME_COMPONENT_PARAMS["steering_wheels"])
            ...
            if local_path.exists():
              ...
              if expected_size > 0 and local_size != expected_size:
                print(f"{component_name} {theme_name} is outdated, redownloading...")
                self.download_theme(component_name, theme_name, THEME_COMPONENT_PARAMS[component_name])
      ...
      print(f"Fetching theme assets: {api_url}")
      response = self.session.get(api_url, timeout=10)
      response.raise_for_status()
      content = response.json()

      if is_github:
        theme_items = content.get("tree", [])
      else:
        theme_items = [item for item in content if item.get("type") == "blob"]
          ...
          if key in item_path:
            ...
            if local_path.exists():
              ...
              if local_size != expected_size:
                ...
                self.download_theme(key, theme_name, THEME_COMPONENT_PARAMS[key])
            break

      return {**assets, "themes": {key: list(value) for key, value in assets["themes"].items()}}
  ...
  def handle_verification_failure(self, extension, theme_component, theme_name, asset_param, theme_path, download_path):
    if theme_component == "steering_wheels":
    ...
    if asset_mappings != self.previous_asset_mappings:
      for asset, (asset_type, current_value) in asset_mappings.items():
        ...
        if asset_type == "wheel_image":
          self.update_wheel_image(current_value, self.holiday_theme, random_event=False)
        else:
          self.update_theme_asset(asset_type, current_value, self.holiday_theme)
  ...
  def validate_themes(self, downloadable_colors, downloadable_distance_icons, downloadable_icons, downloadable_signals, downloadable_sounds, downloadable_wheels, frogpilot_toggles):
    ...
    for display_name, components in downloaded_data.get("themes", {}).items():
      ...
      for component in components:
        ...
        if not component_path.is_dir() or not any(component_path.iterdir()):
          ...
          self.download_theme(component, theme_folder_name, THEME_COMPONENT_PARAMS.get(component))
    ...
    for display_name in downloaded_data.get("steering_wheels", []):
      ...
      if not matching_files:
        ...
        self.download_theme("steering_wheels", file_stem, THEME_COMPONENT_PARAMS["steering_wheels"])
        self.update_active_theme(True, frogpilot_toggles)
